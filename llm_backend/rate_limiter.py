...
from .config import config, APIKeyInfo, RateLimitConfig
...
class RateLimiter:
    ...
    def check_rate_limit(self, user_info: APIKeyInfo) -> None:
        ...
        if len(history) >= limits.requests_per_hour:
            raise HTTPException(
                ...
                detail=f"Rate limit exceeded. Maximum {limits.requests_per_hour} requests per hour allowed for tier '{tier}'.",
                headers={
        ...
        if recent_requests >= limits.requests_per_minute:
            ...
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail=f"Rate limit exceeded. Maximum {limits.requests_per_minute} requests per minute allowed for tier '{tier}'.",
                headers={
    ...
    def get_rate_limit_status(self, user_info: APIKeyInfo) -> Tuple[int, int, int, int]:
        ...
        return (
            ...
            max(0, limits.requests_per_minute - recent_requests),
            limits.requests_per_hour,
            max(0, limits.requests_per_hour - hourly_requests),
        )
